...
fn main() {
    ...
    for (i, line) in fs::read_to_string(file_path).unwrap().lines().enumerate() {
        ...
        if i % 3 == 2 {
            let mut common_badge_items = Vec::new();
            for each_item in elves_vec[0].contents.chars() {
                if elves_vec[1].contents.contains(each_item) {
                    common_badge_items.push(each_item);
                }
            }
            let mut common_item: char = '0';
            for each_item in common_badge_items {
                if elves_vec[2].contents.contains(each_item) {
                    common_item = each_item;
                }
            }
            badge_items.push(common_item);
        }
    }
    println!("{:?}", badge_items);
