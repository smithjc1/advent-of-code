...
fn main() {
    ...
    for line in fs::read_to_string(file_path).unwrap().lines() {
        ...
        if get_lower_assignment(elf_1_assignment) >= get_lower_assignment(elf_2_assignment)
            && get_lower_assignment(elf_1_assignment) <= get_higher_assignment(elf_2_assignment)
        {
            total_overlap = total_overlap + 1;
        } else if get_lower_assignment(elf_2_assignment) >= get_lower_assignment(elf_1_assignment)
            && get_lower_assignment(elf_2_assignment) <= get_higher_assignment(elf_1_assignment)
        {
            total_overlap = total_overlap + 1;
        }
        println!("{}", total_overlap);
